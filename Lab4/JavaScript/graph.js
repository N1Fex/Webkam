...
const COLORS_YEAR = {
    "price": ["#901070", "#BB6060"],
    "volume": ["#AAAA10", "#90BB90"],
    "capital": ["#101090", "#6090BB"]
}
...
function drawGraph(data) {
    // значения по оси ОХ
    const keyX = d3.select("input[name='x-coord']:checked").attr("value");

    let params = []
    d3.selectAll("input[name='y-coord']:checked")._groups[0].forEach(d => params.push(d.value))

    if (params.length === 0){
        d3.selectAll("input[name='y-coord'")._groups[0].forEach(el => el.parentNode.classList.add("error"))
        return;
    }
    d3.selectAll("input[name='y-coord'")._groups[0].forEach(el => el.parentNode.classList.remove("error"))
    ...
    if (attr_area.isYear) {
        ...
        params.forEach( (p, i) => {
            for (let j = 1; j >= 0; j--) {
                switch (type) {
                    case "point":
                        makeCircles(svg, data, r, scaleX, scaleY, attr_area, COLORS_YEAR[p][j], i, j)
                        break;
                    case "line":
                        makeLines(svg, data, scaleX, scaleY, attr_area, COLORS_YEAR[p][j], i, j)
                        break;
                    case "bar":
                        makeBars(svg, data, scaleX, scaleY, attr_area, COLORS_YEAR[p][j], i, j)
                }
            }
